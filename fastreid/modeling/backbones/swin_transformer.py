...
model_urls = {
    ...
    'swin_s3_tiny_224': 'https://github.com/silent-chen/AutoFormerV2-model-zoo/releases/download/v1.0.0/S3-T.pth',
    'swin_s3_small_224': 'https://github.com/silent-chen/AutoFormerV2-model-zoo/releases/download/v1.0.0/S3-S.pth',
    'swin_s3_base_224': 'https://github.com/silent-chen/AutoFormerV2-model-zoo/releases/download/v1.0.0/S3-B.pth'
}
...
class SwinTransformer(nn.Module):
    ...
    def init_weights(self, depth, pretrain=None, pretrain_path=None):
        ...
        if pretrain:
            ...
            if pretrain_path:
                ...
                ckpt = torch.hub.load_state_dict_from_url(
                    model_urls[depth], progress=True, check_hash=True,
                    map_location=torch.device('cpu'))

        if not isinstance(ckpt, dict):
            raise RuntimeError(
                f'No state_dict found in checkpoint file {model_urls[depth]}')
...
def build_swin_backbone(cfg):
    ...
    if depth == 'swin_tiny_patch4_window7_224':
        model = SwinTransformer(embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24], window_size=7, ape=False,
            drop_path_rate=0.2, patch_norm=True, use_checkpoint=False, frozen_stages=0)

    model.init_weights(depth, pretrain, pretrain_path)
    return model
